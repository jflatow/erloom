...
wait() ->
    receive
        {{new_message, Message, Reply}, State} ->
            ...
            State3 = loom:point_to_front(node(), State2),
                ...
                case erloom:edge_delta(Target, Front) of
                    ...
                    _ ->
                        ...
                        maps:fold(fun (Node, Range, S) ->
                                          loom:replay_log(Replay, Range, Node, S)
                                  end, State, TP)
                end;
            _ ->
                %% target is reached
...
write_through({W, T}, Message, State = #{peers := Peers}) when W > 0 ->
    %% write to our own log, and push the entries right away
    {EntryList, State1} = loom:write_log(Message, State),
    State2 = erloom_sync:do_push(#{node() => EntryList}, State1),
